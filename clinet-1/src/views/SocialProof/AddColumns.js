...
// ** Note Import
import Note from "../../views/contacts/employee/list/Note";
// ** Reactstrap Imports
import {
  Badge,
...
const ExpandableTable = ({ data }) => {
  ...
  let empTasks = taskStore?.taskList?.data?.employeeTasks?.find(
    (x) => x._id === data._id
  )?.tasks;
  ...
  function handleRemindClick(item) {
    ...
    dispatch(
      updateTaskStatusUserAction(item._id, {
        status: "remind",
        empId: item.empId,
      })
    );
            ...
            empTasks?.map((item) => {
              return (
                <tr>
                  ...
                  <th>
                    <p className="desEmpText">
                      {item.originTask[0].description}
                    </p>
                  </th>
                  <th style={{ fontSize: "13px", fontWeight: "400" }}>
