...
import { Button, ButtonGroup, Nav, NavItem, NavLink, Offcanvas, OffcanvasBody, OffcanvasHeader } from 'reactstrap';
...
export default function Editor({
  ...
  useEffect(() => {
    ...
    const gjsEditor = grapesjs.init({
      ...
      canvas: {
        styles: [
          ...
          '/assets/form-builder/themes.scss',
        ],
        scripts: [
    ...
    if (step && step.html && step.css) {

      gjsEditor.setComponents(step.html);
    ...
    gjsEditor.Commands.add('set-data', (editor) => {
      const formData = store.form.formData.map(x => {
        if (x.id === step.id) {
          return { ...x, html: editor.getHtml(), css: editor.getCss() }
        }
        return x
      })
      dispatch(
          ...
          if (child.classList.contains('add-new-column')) {
            child.addEventListener('click', (e) => {
              ...
              const filtered = blocks.filter(
                (block) => block.get('category').attributes.id === 'Column'
              );
  ...
  return (
    <>
      ...
      <div>
        <Offcanvas isOpen={isStyles} toggle={() => toggleStyles(!isStyles)} backdropClassName="backdrop" direction="end" style={{width:"400px"}}>
          {/* <OffcanvasHeader toggle={()=>toggleStyles(!isStyles)}>Styles for {stylesTitle}</OffcanvasHeader> */}
          <OffcanvasHeader toggle={() => toggleStyles(!isStyles)}

          >{stylesTitle} Setting</OffcanvasHeader>
      ...
      >
        <div>
          <div className="w-100">
            <div className="d-flex justify-content-between">
              <h6 className="my-auto">Styles for {stylesTitle}</h6>
