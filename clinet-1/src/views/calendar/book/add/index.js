...
const AddBooking = () => {
    ...
    let timeList = []
    for(var i = 60 * 8; i <= 60 * 22 + 30; i += 30) {
        timeList.push(i)
    }
            ...
            <div className='app-booking-container'>
                ...
                <div className={showTime?'app-booking':'app-booking-small'}>
                    ...
                    <Row>
                            ...
                            {store.detailBooking && store.detailBooking._id? (
                                <div>
                                    <div className='mt-1'>
                                        ...
                                        <span className="fw-bold">{formatLocaleTime(new Date(store.detailBooking.startDate), store.detailBooking.timezone)}</span>
                                    </div>

                                    <h4 className='form-check-label fw-bolder mt-1'>
                                        {store.detailBooking.name}
                                    </h4>
                                    <Button className='mt-1' type='reset' color='secondary' outline onClick={() => {
                                        history.goBack();
                                    }}>
                                        Cancel
                                    </Button>
                        ...
                        {showTime? (<Col md="3" sm="12" className='react-slidedown' style={{maxHeight: '500px', overflow: 'auto'}}>
                            <div className='mt-4'>
                                    ...
                                    if(selectTime != time) {
                                        ...
                                        return <Row>
                                            ...
                                            <Col sm='6'>
                                                <Button
                                                    onClick={(e) => {
                                                        var invdate = new Date(picker.toLocaleString('en-US', {
                                                            timeZone: timezone
                                                        }))
                ...
                <ModalData open={sidebarOpen} toggleSidebar={toggleSidebar} timezone={timezone} detailBooking={store.detailBooking} duration={store.detailBookingType && store.detailBookingType.duration} startDate={startDate}/>
            </div>
