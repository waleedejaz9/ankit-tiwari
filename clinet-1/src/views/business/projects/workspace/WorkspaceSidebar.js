...
const WorkspaceSidebar = ({ collapse, addWorkspace, handleWorkspaceCollapse }) => {
    ...
    useEffect(() => {
        ...
        if (currentWorkspaceID) {
            let updatedWorkspace = workspaceData.find((workspace) => workspace._id === currentWorkspaceID)
        ...
        <div className="project-sidebar" style={{ width: '250px', height: '100%' }}>
            <div className="sidebar-content task-sidebar">
                <div className="task-app-menu">
                    <ListGroup className={`sidebar-menu-list ${collapse ? 'd-none' : 'd-block'}`} options={{ wheelPropagation: false }}>
                        <div className="py-1 ps-1 d-flex justify-content-between align-items-center" style={{ marginLeft: '0.5rem' }}>
                        ...
                        {workspace?.map((workspace, index) => {
                            return (
                                <ListGroupItem
                                    key={workspace._id}
                                    active={workspace._id === currentWorkspaceID || (index === 0 && !currentWorkspaceID) ? true : false}
                                    onClick={((e) => {
                                        e.preventDefault();
