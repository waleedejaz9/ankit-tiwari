...
import { deleteMemberFileAction } from '../../store/actions'
import { store } from '@store/store'
const onDelete = async (member, id) => {
    const result = await confirm('Are you sure?', {
        closeOnOverlayClick: true,
        classNames: 'custom_confirm_box'
    })
    if (result) {
        store.dispatch(deleteMemberFileAction(member, id))
        return
    }
}
