...
// Actions
import { uploadFilesAction, deleteMemberFileAction } from '../../store/actions'
...
const FilesTab = ({ selectedUser }) => {
    ...
    const {
        ...
        isSuccess,
        error: reqeustError
    } = useSelector((state) => state?.memberContact?.filesUpload)

    const { isSuccess: deleteIsSuccess, isLoading: isDeleteLoading } =
        useSelector((state) => state?.memberContact?.fleUplaodDelete)
    ...
    function handleFileUPload() {
        ...
        if (title === '') {
            ...
            form.append('memberId', selectedUser?._id)
    ...
    const handleRankDelete = () => {
        dispatch(deleteMemberFileAction(selectedUser?._id, deleteId))
    }
