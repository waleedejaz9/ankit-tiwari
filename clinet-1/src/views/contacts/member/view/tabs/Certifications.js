...
const Certifications = ({ selectedUser }) => {
    ...
    const value = useSelector((state) => state?.memberContact)
    const {
        singleMember,
        rank: { isSuccess },
    ...
    const { member } = singleMember ? singleMember : []
    const { ranks } = member ? member : []
    ...
    function handleAddRank() {
        ...
        if (rankName === '') {
            ...
            formData.append('memberId', selectedUser?._id)
    ...
    const handleRankDelete = () => {
        dispatch(deleteRankAction({ memberId: id, rankId: deleteId }))
    }

    useMemo(() => {
        if (deleteSuccess) {
            fetchSingleMemberAction(id)
