...
import { editMemberReset } from '../store/reducer';
// ** Third Party Components
import Select from 'react-select';
import Flatpickr from 'react-flatpickr';
import { Check, X, Users } from 'react-feather';
import { useForm } from 'react-hook-form';
import { fetchSingleMemberAction, editMemberAction, uploadAvatarAction } from '../store/actions';
...
const UserInfoCard = ({ selectedUser }) => {
  ...
  const { contact, isMemberContactEditSuccess, isMemberContactLoading } = useSelector(
    (state) => state.memberContact
  );
  ...
  const handleEditSubmit = () => {
    ...
    dispatch(editMemberAction(id, preparedData));
  };

  useMemo(() => {
    if (isMemberContactEditSuccess) {
      toast.success(<ToastContent message="Members updated successfull" />);
      dispatch(editMemberReset());
      setShow(false);
    }
  }, [isMemberContactEditSuccess]);
    ...
    <Fragment>
      ...
      <Card>
        <CardBody>
          ...
          <Row className="d-flex justify-content-around my-2 pt-75">
            <Col className="d-flex align-items-start me-2">
              <Badge color="light-primary" className="rounded p-75">
                <BsHouse className="font-medium-2" />
              </Badge>
              <div className="ms-75">
          ...
          !selectedUser.companyAddress ? null : (
            <>
      ...
      <Modal isOpen={show} toggle={() => setShow(!show)} className="modal-dialog-centered modal-lg">
        ...
        <ModalBody className="px-sm-5 pt-50 pb-5">
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Row className="gy-1">
              ...
              <Col md={6} xs={12}>
                ...
                <Select
                  ...
                  placeholder="Select"
                  defaultValue={{
                    value: contact?.gender ? contact?.gender : 'Select One',
                    label: contact?.gender
                      ? contact?.gender?.charAt(0)?.toUpperCase() + contact?.gender?.substring(1)
                      : 'Select One'
                  }}
                />
              </Col>
              <Col md={6} xs={12}>
                ...
                <Select
                  ...
                  placeholder="Select"
                  defaultValue={{
                    value: contact?.status ? data?.status : 'Select One',
                    label: contact?.status
                      ? contact?.status?.charAt(0)?.toUpperCase() + contact?.status?.substring(1)
                      : 'Select One'
                  }}
                />
              </Col>
              ...
              <Col md={6} xs={12}>
                ...
                <Select
                  ...
                  className="react-select"
                  classNamePrefix="select"
                  options={memberTypeptions}
                  theme={selectThemeColors}
                  onChange={(e) => handleSelectChange('type', e.value)}
                  placeholder="Select"
                  defaultValue={{
                    value: contact?.type ? contact?.type : 'individual',
                    label: contact?.type
                      ? contact?.type?.charAt(0)?.toUpperCase() + contact?.type?.substring(1)
                      : 'Individual'
                  }}
                />
              </Col>
            ...
            <div className="d-flex justify-content-between align-itmes-center border-bottom mb-1 mt-3">
              <InputGroup className="d-flex justify-content-between pb-1">
                <h4 className="fw-bolder ms-1">Company Info</h4>
                <div className="d-flex align-items-center gap-1 me-1">
                  <Input type="checkbox" id="toggle-display-profile" onClick={toggle} addon />
                  <label className="m-0" htmlFor="toggle-display-profile">
              ...
              <Row>
                ...
                <Col md={6} xs={12}>
                  ...
                  <Select
                    ...
                    defaultValue={{
                      value: contact?.position ? contact?.position : 'Select One',
                      label: contact?.position
                        ? contact?.position?.charAt(0)?.toUpperCase() +
            ...
            <Row>
              ...
              <Col xs={12} className="text-center mt-2 pt-50">
                <Button
                  ...
                  disabled={isMemberContactLoading}
                >
                  {isMemberContactLoading ? 'Processing...' : 'Submit'}
                </Button>
                <Button
                  type="reset"
                  color="secondary"
