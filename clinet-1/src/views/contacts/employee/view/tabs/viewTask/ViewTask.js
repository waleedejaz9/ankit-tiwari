...
import { updateTaskByEmployeeAction, updateTaskByUserAction } from '../../../../../settings/tabs/rolesandper/store/employee/action';
...
export default function ViewTask({ open, toggle, empTask }) {
  ...
  const handleDownload = async () => {
    ...
    if(attachments.length>0){
      attachments.map(x=>{
       if(x.isDone===true){
        FileSaver.saveAs(x.list[0],`attachment-${empTask.originTask[0].title}-${x.recipient.name}.pdf`)
       }
      })
    }
  };
  ...
  return (
  <>
    {(empTask && empTask.originTask) && <Modal isOpen={open} toggle={toggle} fullscreen={empTask?.originTask[0]?.type === 'form'} size="lg">
      ...
      <div className="container-fluid">
        <Row>
          <Col md="8">
            <div className="d-flex justify-content-start">
              ...
              <div className="p-1">
                ...
                <Label className="text-secondary">{empTask?.originTask[0]?.description}</Label>
              </div>
            </div>
          </Col>
          ...
          <Col md="4" className="my-auto text-end">
            <Button color="primary" className='me-1' onClick={handleSave}>Save</Button>
    ...
    <ModalBody>{(empTask?.originTask[0]?.type === 'form' )? <DocViewer task={empTask} attachments={attachments} setAttachments={setAttachments} boardDimention={boardDimention} setBoardDimention={setBoardDimention}/>:<UploadDoc attachments={attachments} setAttachments={setAttachments}/>}</ModalBody>
  </Modal>}
  </>
  );
}
