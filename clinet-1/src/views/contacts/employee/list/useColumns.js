...
import ViewTask from '../view/tabs/viewTask/ViewTask';
import { updateTaskByUserAction, updateTaskStatusUserAction } from '../../../settings/tabs/rolesandper/store/employee/action';

export const useColumns = ({ setDeleteModal }) => {
  
  // let count = [];
  // taskStore.taskList.map((x) => {
  //   for (const i of x.empTaskStatus) {
  //     count.push({ empId: i.empId, status: i.status, taskId: x._id });
  //   }
  // });
  ...
  //   let temp = count.reduce((counts, { empId, status }) => {
  //     if (!counts[empId]) counts[empId] = [];
  //     counts[empId].push(status);
  //     return counts;
  //   });
...
const ExpandableTable = ({ data }) => {
  ...
  function handleDenyClick(item) {
    ...
    dispatch(updateTaskStatusUserAction(item._id,{status:'declined',empId:item.empId,note:note}))
  }
  ...
  return (
    <div className="expandable-content" style={{ marginLeft: '5%', marginRight: '10%' }}>
      <Table className="employee-sub-menu-table">
        ...
        <tbody>
          {empTasks && empTasks?.map((item) => {
            return (
              <tr>
                <th>
                  <p style={{ fontSize: '13px', fontWeight: '600' }}>{item.originTask[0].title}</p>
                </th>
                ...
                <th style={{ fontSize: '13px', fontWeight: '400' }}>{item?.history[item.history.length-1].note}</th>
                <th>
                    ...
                    <Modal isOpen={modal} toggle={handleNotes} centered>
                      ...
                      <ModalBody>
                        <FormGroup>
                          ...
                          <Input id="exampleText" name="text" type="textarea" onChange={handleNoteChanged}/>
                        </FormGroup>
                      </ModalBody>
