...
const EmployeeList = () => {
  ...
  const tableData = empData?.map(x=>{
    const approved = storeTasks?.taskList?.data?.approvedEmployee.find(y=>y._id===x._id)?.approved || 0
    const total = storeTasks?.taskList?.data?.employeeTasks.find(y=>y._id===x._id)?.tasks?.length
