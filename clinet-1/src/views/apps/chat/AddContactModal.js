...
const AddContactModal = ({ modal, setModal, addContactModalToggle, selectedRow }) => {
    ...
    const contactClickHandler = (contact) => {
        if (contact == "employee") {
            ...
            if (status?.employeeContact.addEmployee.error) { error(`Adding ${selectedContact.email} failed!   ` + status?.employeeContact.addEmployee.error) } else { success(`Add ${selectedContact.email} to Employee Successfully`) };
        } else if (contact == "lead") {
            dispatch(addLeadContactAction(selectedContact));
            if (status?.leadContact.addLead.error) { error(`Adding ${selectedContact.email} failed!   ` + status?.leadContact.addLead.error) } else { success(`Add ${selectedContact.email} to Lead Successfully`) };
        } else if (contact == "relationship") {
            dispatch(addRelationContactAction(selectedContact));
            if (status?.relationshipContact.addLead.error) { error(`Adding ${selectedContact.email} failed!   ` + status?.relationshipContact.addLead.error) } else { success(`Add ${selectedContact.email} to Relationship Successfully`) };
        } else if (contact == "vendor") {
            dispatch(addVendorContactAction(selectedContact));
            if (status.vendorContact.addLead.error) { error(`Adding ${selectedContact.email} failed!   ` + status.vendorContact.addLead.error) } else { success(`Add ${selectedContact.email} to Vendors Successfully`) };
        } else if (contact == "client") {
            dispatch(addClientContactAction(selectedContact));
            if (status.clientContact.clientContact.error) { error(`Adding ${selectedContact.email} failed!   ` + status.clientContact.isClientContactErrors.error?.data.message) } else {
