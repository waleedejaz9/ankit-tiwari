...
exports.listenIncomingSMS = async (req, res) => {
  ...
  const clientObjs = await ClientContact.find({
    phone: from.split("+")[1],
  });
  ...
  let textMsg = new textMessage(
    Object.assign({}, {isSent: false, textContent: msg, userId: userObj.user_id, uid: clientObj._id.toString(), email: clientObj.email}, {
      time: new Date().toLocaleString("en-US", {
        timeZone: "America/New_York",
      }),
    })
