...
exports.getClientContacts = async (req, res) => {
  try {
    ...
    for (let i = 0; i < clientContacts[0].data.length; i++){
      let unReadMeassges = await TextMessages.find({ uid: clientContacts[0].data[i]._id, isRead: false })
      clientContacts[0].data[i].unReadMessages = unReadMeassges.length
    }
    
    clientContacts[0].data.sort((first, second) => second?.lastMessage?.createdAt - first?.lastMessage?.createdAt)
